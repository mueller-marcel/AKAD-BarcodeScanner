...
def decode_barcode(image: np.ndarray) -> str | None:
    ...
    scanlines = _extract_scanlines(image)

    for scanline in scanlines:
        # Get the edge signal
        gradient = _normalize_gradient(scanline)

        # Find the edges
        edges = _detect_edges(gradient)

        # Estimate the width between the fixed edges
        delta = _estimate_module_width(edges)

        # Get the fixed edges
        fixed_edges = _viterbi_edges(gradient, edges, expected_num_edges=60, delta=delta)

        # Get the left digits together with their patterns
        left_digits, pattern = _viterbi_digit_decoding_left(fixed_edges, delta=delta)

        # Get the right digits
        right_digits = _viterbi_digit_decoding_right(fixed_edges, delta=delta)
        ...
        ean_13_parity_digit = PARITY_SEQUENCE_MAPPING.get(parity_pattern) if parity_pattern in PARITY_SEQUENCE_MAPPING else 0
    ...
    # Verify the checksum
    is_correct = _verify_checksum(final_digits)
...
def _extract_scanlines(image: np.ndarray, num_lines=1) -> list[np.ndarray]:
    """
...
def _normalize_gradient(scanline: np.ndarray) -> np.ndarray:
    """
...
def _detect_edges(gradient: np.ndarray) -> np.ndarray:
    """
...
def _estimate_module_width(edges: np.ndarray) -> float:
    """
...
def _viterbi_edges(gradient: np.ndarray, edge_candidates: np.ndarray, expected_num_edges=60, delta=None) -> np.ndarray:
    """
...
def _viterbi_digit_decoding_left(edge_positions: np.ndarray, delta: float):
    """
    ...
    for i in range(12):
        ...
        candidates = _generate_digit_candidates_left(widths)
    ...
    for pattern in pattern_sequence:
        patterns.append(LEFT_EDGES_TO_CODE_MAPPING.get(pattern))

    return list(sequence), list(patterns)


def _viterbi_digit_decoding_right(edge_positions: np.ndarray, delta: float):
    """
    ...
    for i in range(6):
        base = start + i * 4
        widths = np.diff(edge_positions[base:base + 5]) / delta
        candidates = _generate_digit_candidates_right(widths)
        digit_candidates.append(candidates)
...
def _generate_digit_candidates_left(widths: np.ndarray) -> list:
    """
    ...
    candidates = []
    for pattern, digit in LEFT_EDGES_TO_DIGIT_MAPPING.items():
...
def _generate_digit_candidates_right(widths: np.ndarray) -> list:
    """
    ...
    candidates = []
    for pattern, digit in RIGHT_EDGES_TO_DIGIT_MAPPING.items():
        error = np.sum((np.array(widths) - np.array(pattern)) ** 2)
...
def _verify_checksum(digits: list) -> bool:
    """
